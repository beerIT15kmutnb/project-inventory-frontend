...
export class UnitManagementComponent implements OnInit {
  ...
  async saveEdit() {
    ...
    const rs: any = await this.productsService.editUnit(this.items);
    if (rs.ok) {
  ...
  async saveAdd() {
    console.log(this.items);
    const rs: any = await this.productsService.addUnit(this.items);
    if (rs.ok) {
      this.openModal = false;
      this.items = [];
      this.alertService.success();
  ...
  async getUnit() {
    const rs: any = await this.productsService.getUnit();
    if (rs.ok) {
      this.units = rs.rows;
    } else {
      this.alertService.error(rs.error);
