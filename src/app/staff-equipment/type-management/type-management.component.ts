...
export class TypeManagementComponent implements OnInit {
  ...
  async saveEdit(){
    ...
    let rs:any = await this.equipmentsService.editType(this.items)
    if(rs.ok){
  ...
  async saveAdd(){
    console.log(this.items);
    let rs:any = await this.equipmentsService.addType(this.items)
    if(rs.ok){
      this.openModal = false
      this.items = []
      this.alertService.success()
  ...
  // async getType() {
  //   let rs: any = await this.equipmentsService.getType()
  //   if (rs.ok) {
  //     this.types = rs.rows
  //   } else {
  //     this.alertService.error(rs.error)
  ...
  setisActive(active: any, id: any) {
    ...
    this.equipmentsService.isActive(id, status)
      .then((result: any) => {
        if (result.ok) {
          this.alertService.success();
        } else {
