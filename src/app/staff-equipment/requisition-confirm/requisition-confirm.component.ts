...
export class RequisitionConfirmComponent implements OnInit {
    ...
    try {
      ...
      if (rs.ok) {
        ...
        for (let v of this.products) {
          await this.alowcate(v.product_id);
        }
  ...
  async editChangereqQty(idx: any, qty: any) {
    if ((+qty.value) > +this.products[idx].remain_qty) {
      ...
      await this.alowcate(this.products[idx].product_id);
  ...
  async save() {
    ...
    this.alertService.confirm('ต้องการยืนยันการเบิก ใช่หรือไม่?')
      .then(async () => {
        ...
        this.products.forEach(v => {
          const total = v.requisition_qty;
          if (total > v.remain_qty || v.requisition_qty <= 0) {
            isError = true;
          }
        });
        ...
        if (isError) {
          ...
          this.modalLoading.show();
          try {
            let rs: any = await this.requisitionService.approveRequisitionOrder(this.requisitionId, this.products);
            this.modalLoading.hide();
            this.isSave = false;
            if (rs.ok) {
              this.alertService.success()
              this.router.navigate(['/staff-equipment/requisition']);
            } else {
              this.alertService.error(rs.error);
            }
  ...
  changeSearchEquipment(event: any) {
    // if (event) {
    //   this.clearItem();
    // }
  }
  ...
  clearItem() {
    ...
    // this.searchEquipmentCmp.clearSearch();
  }
  ...
  // this.selectedWorkingCode = equipment.working_code;
  // this.selectedRemainQty = equipment.qty;
  // this.selectedRequisitionQty = 1;

  // this.selectUnits.getUnits(equipment.equipment_id);
  // }
  ...
  //   product.equipment_id = this.selectedEquipmentId;
  //   product.requisition_qty = this.selectedRequisitionQty;
  //   product.equipment_name = this.selectedEquipmentName;
  //   product.to_unit_qty = this.selectedSmallQty;
  //   product.unit_equipment_id = this.selectedUnitEquipmentId;
  ...
  async getTemplateItems(templateId: any) {
    ...
    //     rs.rows.forEach(v => {
    //       const product: IRequisitionOrderItem = {};
    //       product.equipment_id = v.equipment_id;
    //       product.requisition_qty = 0;
    //       product.equipment_name = v.equipment_name;
    //       product.to_unit_qty = 0;
    //       product.unit_equipment_id = v.unit_equipment_id;
  ...
  async getEquipmentItems(event: any) {
    // if (this.templateId) {
