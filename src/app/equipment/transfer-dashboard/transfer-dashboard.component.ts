...
export class TransferDashboardComponent implements OnInit {
  ...
  addition() {
    this.router.navigate(['/equipment/addition'], { queryParams: { equipments: JSON.stringify(this.selectedApprove) } });
    // routerLink="/equipment/addition" [queryParams]="{equipments: selectedApprove}"
  }
  ...
  print(item: any) {
    const url = `${this.apiUrl}/report/addEqui/${item.addition_id}`;
    // ?token=${this.token}`;
    console.log(url);
    this.htmlPreview.showReport(url, 'landscape');
  }

  async print2(item: any) {
    this.modalLoading.show();
    this.openModal = true
    try {
  ...
  toCSV(tableId) {
    this.filename = 'ประมาณการวัสดุสำนักงาน ปราจีนบุรี งบ '+this.exportExcel.printMount;
