...
export class ProductsService {
  ...
  async isActiveProduct(id: any, isActive: any) {
    let res = await this.authHttp.put(`${this.url}/equipment-products/is-active-product`, {
      id: id,
      is_active: isActive
    }).toPromise();
    return res.json();
  ...
  async isActiveEquipment(id: any, isActive: any) {
    let res = await this.authHttp.put(`${this.url}/equipment-products/is-active-equipment`, {
      id: id,
      is_active: isActive
    }).toPromise();
    return res.json();
  ...
  async allEquipments(query: any, limit: number = 10, offset: number = 0) {
  ...
  async search(query: any, equipmentType: any, limit: number = 10, offset: number = 0) {
  ...
  async addUnit(items:any){
    let rs:any =  await this.authHttp.put(`${this.url}/equipment-products/addUnit`,{
      items:items
    }).toPromise();
    return rs.json();
  }
  async editUnit(items:any){
    let rs:any =  await this.authHttp.put(`${this.url}/equipment-products/editUnit`,{
      items:items
    }).toPromise();
    return rs.json();
  }
